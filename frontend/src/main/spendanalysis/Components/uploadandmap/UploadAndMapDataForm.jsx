...
function FormPopUp(props) {
  ...
  const defaultTempleteSelectHandle = (e) => {
    ...
    let defaultTemplateReference = parseInt(
    ...
    defaultTemplates.forEach((element) => {
      if (element.id === defaultTemplateReference) {
        dispatch(setMapSelectedDefaultTemplateData(JSON.parse(element.Items)));
      }
    });
  };
            ...
            <Grid xs={12} sm={12} md={12} lg={12} xl={12}>
              {Object.keys(selectedDefaultTemplateData).map((sensitivity) => (
                <>
                  ...
                  {Object.keys(selectedDefaultTemplateData[sensitivity]).map(
                    (type) => (
                      <>
                        <Box>{type}</Box>
                        {Object.keys(
                          selectedDefaultTemplateData[sensitivity][type]
                        ).map((element) => (
                          <Grid container spacing={0}>
                            <Grid xs={6} sm={6} md={6} lg={6} xl={6}>
                              <Box className={classes.itemKeys}>{element}</Box>
                            </Grid>
                            <Grid xs={6} sm={6} md={6} lg={6} xl={6}>
                              <DropDownSelect
                                ...
                                fieldName="TableName"
                                selectedValue={
                                  selectedDefaultTemplateData[sensitivity][
                                    type
                                  ][element]
                                }
                                label="Select"
                                onChangeEvent={(e) =>
                                  mappingItemSelectHandle(
                                    e,
                                    sensitivity,
                                    type,
                                    element
