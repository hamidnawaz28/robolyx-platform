  ...
  if (Lists) {
    if (dataType === "KeyValue") {
      allOptions = Lists.map((ListItem) => (
        <option pk={ListItem["pk"]} className="selectOption">
          {ListItem["name"]}
        </option>
      ));
    }
    if (dataType === "ListData") {
      allOptions = Lists.map((ListItem) => (
        <option pk={ListItem} className="selectOption">
          {ListItem}
        </option>
      ));
    }
    ...
    if (dataType === "ListDataWithFieldName") {
      allOptions = Lists.map((ListItem) => (
        <option fieldName={fieldName} pk={ListItem} className="selectOption">
          {ListItem}
        </option>
      ));
    }
    if (dataType === "GroupDataWithType") {
      allOptions = Object.keys(Lists).map((ListItem) =>
        Lists[ListItem].map((item) => (
          <option dataType={ListItem} className="selectOption">
            {item}
          </option>
        ))
      );
    ...
    if (dataType === "WithPkAndFieldName") {
      allOptions = Lists.map((ListItem) => (
        <option key={ListItem.pk} id={ListItem.id} className="selectOption">
          {ListItem?.fields?.[fieldName] ? ListItem.fields[fieldName] : <></>}
        </option>
      ));
    }
    if (dataType === "GroupData") {
      ...
      allOptions = SensitivitiesLevels.map((SensitivityLevel) => (
        <optgroup
          ...
          className={SensitivityLevel + "Options"}
        >
          {Object.keys(Lists[SensitivityLevel]).map((data) => (
            <>
              {Lists[SensitivityLevel][data].map((value) => (
                <option data={[SensitivityLevel, data, value]}>{value}</option>
              ))}
            </>
          ))}
        </optgroup>
